...
function toggleHighlight(markerView, property) {
  ...
  if (markerView.content.classList.contains("highlight")) {
	...
	$('.markIcon'+property.pid).show();
	//opt2 $('.allprops').show();
  } else {
    markerView.content.classList.add("highlight");
    markerView.zIndex = 100000;
	$('.markIcon'+property.pid).hide();
  }
}
...
function buildContent(property) {
  ...
  /*
  content.innerHTML = `
    <div class="markIcon${property.pid}" style="background-image: url(img/icons/pin-white-map.svg);width:45px;height:55px;">
		<div class="markerDeg">${property.deg}</div>
		<img src="img/icons/${property.icon}" class="marker-ico${property.icoex}" />
		<div class="markerName">${property.name}</div>
    </div>
    <div class="details">
		<div class="close"><img src="img/icons/close.png" /></div>
        <div class="name">${property.name}</div>
        <div class="address">${property.address}</div>
        <div class="but" onclick="javascript:location.href='${property.url}';">View full forecast</div>
    </div>
    `;
 */

  content.innerHTML = `
    <div class="allprops markIcon${property.pid}" style="background-image: url(img/icons/pin-dark.svg);width:45px;height:55px;">
		<img src="img/icons/logo-white.svg" class="marker-ico${property.icoex}" />
    </div>
    <div class="details">
		<div class="close"><img src="img/icons/close.png" onclick="javascript:clickedProp(${property.pid});" /></div>
        <div class="name">${property.name}</div>
        <div class="address">${property.address}</div>
